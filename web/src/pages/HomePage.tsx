...
export const HomePage = () => {
  ...
  const { user } = useAuth();
  const { createSession, isCreating, error, reset } = useChatSession();
  ...
  const handleSubmit = async (prompt?: string) => {
    ...
    if (message.trim() && !isCreating) {
      try {
        ...
        const data = await createSession(message);
  ...
  return (
    <Layout showSidebar={true} showTopNav={false} className="flex-1">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {suggestions.map((suggestion) => (
                <Card 
                  key={suggestion.id}
                  className={`cursor-pointer transition-all duration-200 hover:shadow-lg hover:scale-105 group border-muted-foreground/20 hover:border-primary/50 ${isCreating ? 'opacity-50 pointer-events-none' : ''}`}
                  onClick={() => !isCreating && handleSuggestionClick(suggestion)}
                >
                  <CardContent className="p-6">
                    <div className="space-y-4">
